...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (wilber, tux, penny, beastie, gopher) = load_jumpscares(&textures);
    ...
    while !rl.window_should_close() {
        ...
        state.jumpscarer = MonsterName::Penny;
        unsafe {
            SCREEN.update();
        }
                    ...
                    match state.screen {
                            ...
                            if state.getting_jumpscared {
                                ...
                                match state.jumpscarer {
                                    ...
                                    MonsterName::Beastie => {
                                        ...
                                        if state.gameover_time.elapsed()?.as_millis() <= 500 {
                                            d.draw_texture_pro(
                        ...
                        Screen::Camera => {
                            ...
                            let texture = match state.sel_camera {
                                ...
                                _ => {
                                    panic!("tried to draw unsupported room {:?}", state.sel_camera)
                                }
                            };
                    ...
                    for mons in inoffice {
                        if mons.active() {
                                ...
                                >= MONSTER_TIME_OFFICE_WAIT_THING
                            {
                                ...
                                if mons.entered_from_left() || mons.entered_from_right() {
